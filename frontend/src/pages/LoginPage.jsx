...
const S = {
  ...
  panel: {
    ...
    padding: '2.2rem 2.8rem', color: '#fff', position: 'relative', overflow: 'hidden',
  },
  panelOrb1: { position: 'absolute', width: 340, height: 340, borderRadius: '50%', background: 'rgba(255,255,255,0.07)', top: -80, left: -80, pointerEvents: 'none' },
  panelOrb2: { position: 'absolute', width: 220, height: 220, borderRadius: '50%', background: 'rgba(255,255,255,0.05)', bottom: 60, right: -60, pointerEvents: 'none' },
  brandRow: { display: 'flex', alignItems: 'center', gap: '0.6rem', position: 'relative' },
  brandIcon: { width: 38, height: 38, borderRadius: 8, background: 'rgba(255,255,255,0.2)', display: 'grid', placeItems: 'center', fontSize: 18 },
  brandName: { margin: 0, fontFamily: "Sora,'Trebuchet MS',sans-serif", fontSize: '1.3rem', fontWeight: 800, color: '#fff' },
  panelBody: { flex: 1, display: 'flex', flexDirection: 'column', justifyContent: 'center', position: 'relative' },
  panelHeading: { margin: '0 0 1.2rem', fontFamily: "Sora,'Trebuchet MS',sans-serif", fontSize: 'clamp(1.9rem,3.5vw,2.7rem)', fontWeight: 800, lineHeight: 1.15, color: '#fff' },
  ...
  formSide: { background: '#fff', display: 'flex', flexDirection: 'column', padding: '2.2rem clamp(2rem,6vw,4.5rem)' },
  backLink: { alignSelf: 'flex-end', display: 'flex', alignItems: 'center', gap: '0.35rem', color: '#64748b', fontSize: '0.9rem', fontWeight: 600, textDecoration: 'none', transition: 'color 0.18s' },
  formInner: { flex: 1, display: 'flex', flexDirection: 'column', justifyContent: 'center', maxWidth: 420, margin: '0 auto', width: '100%' },
  heading: { margin: '0 0 0.35rem', fontFamily: "Sora,'Trebuchet MS',sans-serif", fontSize: '2rem', fontWeight: 800, color: '#0f172a' },
  ...
  input: { width: '100%', border: '1.5px solid #e2e8f0', borderRadius: 10, padding: '0.8rem 0.9rem 0.8rem 2.5rem', fontSize: '0.95rem', color: '#0f172a', background: '#f8fafc', outline: 'none', transition: 'border-color 0.2s, box-shadow 0.2s, background 0.2s', fontFamily: 'inherit', boxSizing: 'border-box' },
  ...
  oauthBtn: { width: '100%', padding: '0.82rem', borderRadius: 10, border: '1.5px solid #e2e8f0', cursor: 'pointer', fontFamily: 'inherit', fontWeight: 700, fontSize: '0.95rem', background: '#fff', color: '#1e293b', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '0.65rem', transition: 'background 0.18s, transform 0.18s' },
...
export default function LoginPage() {
  ...
  useEffect(() => {
    ...
    authService.oauthInfo()
      .then(data => { if (active) setOauthInfo({ googleLoginUrl: data.googleLoginUrl || '' }); })
  ...
  const handleSubmit = async e => {
    ...
    setSubmitting(true);
    try {
      const loggedUser = await devLogin({ email: form.email.trim().toLowerCase(), password: form.password });
      navigate(loggedUser?.role === 'ADMIN' ? '/app/admin' : '/app/dashboard', { replace: true });
    } catch (err) {
      ...
      <div style={S.page} className="sc-page">
        <div style={S.panel} className="sc-panel">
          ...
          <div style={S.panelBody}>
            ...
            <p style={S.panelSub}>Sign in with your SLIIT campus email to access your bookings, tickets and notifications.</p>
            <ul style={S.featureList}>
              {['Only SLIIT email format is accepted.', 'Example: it23817180@my.sliit.lk', 'JWT based secure access after login.'].map(item => (
        ...
        <div style={S.formSide}>
          ...
          <div style={S.formInner}>
            ...
            <p style={S.sub}>Enter your SLIIT email and password.</p>

            <form onSubmit={handleSubmit} style={S.formGrid} noValidate>
              <label style={S.label}>
                Email address
                <InputField icon={<MailIcon />} type="email" placeholder="it23817180@my.sliit.lk" value={form.email} onChange={e => setForm(previous => ({ ...previous, email: e.target.value }))} hasError={Boolean(fieldErrors.email)} required />
                {fieldErrors.email && <span style={S.fieldError}>{fieldErrors.email}</span>}
              </label>
              <label style={S.label}>
                Password
                <InputField icon={<LockIcon />} showToggle placeholder="••••••••" value={form.password} onChange={e => setForm(previous => ({ ...previous, password: e.target.value }))} hasError={Boolean(fieldErrors.password)} required />
            ...
            <p style={S.footerNote}>Don't have an account? <Link to="/auth/register" style={S.footerLink}>Create an account</Link></p>
          </div>
        </div>
      </div>
    </>
